            ...
            {filteredRooms.map((room, idx) => {
              ...
              return (
                ...
                >
                  ...
                  <div className={`p-4 border-t grid grid-cols-2 gap-2 ${isDark ? 'bg-gray-900/50 border-gray-800' : 'bg-slate-50 border-slate-100'}`}>
                    <button
                      onClick={() => handleOpenNavigation(details.roomName, details.floor)}
                      className={`py-2 px-3 rounded-xl text-xs font-semibold flex items-center justify-center gap-1.5 transition ${isDark ? 'bg-gray-800 hover:bg-gray-700 text-slate-300' : 'bg-white hover:bg-slate-100 text-slate-700 border border-slate-200 shadow-sm'}`}
                    >
                      <Compass className="w-3.5 h-3.5 text-indigo-500" />
                      2D Block
                    </button>
                    <button
                      onClick={() => navigate(`/campus-map?room=${encodeURIComponent(details.roomName)}&block=${encodeURIComponent(details.block)}&floor=${encodeURIComponent(details.floor)}`)}
                      className="py-2 px-3 bg-gradient-to-r from-indigo-600 to-purple-600 hover:from-indigo-500 hover:to-purple-500 text-white rounded-xl text-xs font-semibold flex items-center justify-center gap-1.5 shadow-sm transition active:scale-[0.98]"
                    >
        ...
        <div className="fixed inset-0 z-50 bg-slate-950/80 backdrop-blur-sm flex items-center justify-center p-3 md:p-6 overflow-hidden animate-fadeIn">
          <div className="bg-slate-900 text-white w-full max-w-5xl rounded-3xl border border-slate-800 shadow-2xl overflow-hidden flex flex-col max-h-[92vh]">
            ...
            <div className="p-4 md:p-5 bg-slate-900 border-b border-slate-800 flex items-center justify-between z-10">
              ...
              <div className="flex items-center gap-2">
                <button
                  onClick={() => navigate(`/campus-map?room=${encodeURIComponent(navTargetRoom.name)}&block=${encodeURIComponent(navTargetRoom.block)}&floor=${encodeURIComponent(navTargetRoom.floor)}`)}
